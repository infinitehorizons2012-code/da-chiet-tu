...
start_idx = content.find('function TracNghiemTab({ currentUser')
if start_idx == -1:
    print("Cannot find TracNghiemTab")
    sys.exit(1)

before_tracnghiem = content[:start_idx]

new_tracnghiem = """function TracNghiemTab({ currentUser, userStats, setUserStats, savedSession, setSavedSession }) {
  ...
  const finishSession = async () => {
      ...
      for (const charObj of session.questions) {
          ...
          const newSrs = { ...charObj.srs, [srs_]: { status: newStatus, level: newLevel } };
  ...
  const generateMultipleChoice = (charObj, mode) => {
     ...
     if (mode === 'han_pinyin') {
        questionText = Ch?n Pinyin dúng cho ch?: ;
        ...
        questionText = Ch?n ch? Hán có Pinyin là: ;
  ...
  const loadNextCharacter = async (charObj) => {
    ...
    let compIndex = 0;
    for (let i = 1; i <= 12; i++) {
      const compStr = charObj[App_Comp_];
      if (compStr && compStr !== 'nan' && compStr.trim() !== '') {
        comps.push({ id: App_Comp_, text: compStr.trim(), color: COLORS[compIndex % COLORS.length] });
        compIndex++;
      }
    }
    ...
    try {
      const res = await fetch(https://cdn.jsdelivr.net/npm/hanzi-writer-data@2.0/.json);
      if (res.ok) {
        const data = await res.json();
        setStrokePaths(data.strokes);
      } else {
        setStrokePaths([]);
        setFeedback({ type: 'error', message: Không tìm th?y nét v? cho ch? });
      }
    } catch (err) {
      setStrokePaths([]);
    } finally {
  ...
  const handleMcqSelect = (option) => {
     ...
     if (option === mcq.correctAnswer) {
        ...
        setFeedback({ type: 'error', message: Chua chính xác! Ðáp án dúng là:  });
     }
  };
          ...
          {session.mode === 'chiettu' ? (
            <div className="chiettu-workspace">
              <div className="chiettu-svg-container">
                ...
                {feedback && (
                  <div className={eedback-box } style={{ marginTop: '20px', padding: '15px', borderRadius: '8px', fontWeight: 'bold', background: feedback.type === 'success' ? '#dcfce7' : '#fee2e2', color: feedback.type === 'success' ? '#166534' : '#991b1b' }}>
                    {feedback.message}
                  </div>
                )}
              </div>
...
with open('src/App.jsx', 'w', encoding='utf-8') as f:
    f.write(before_tracnghiem + new_tracnghiem)
print("Patched TracNghiemTab successfully.")
